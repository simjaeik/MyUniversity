...
void insert_to_index_directory(Song *ptr_song)
{
	...
	ptr_snode->song = ptr_song;
	ptr_snode->prev = NULL;
	ptr_snode->next = NULL;		// unused

	int index = ptr_song->index % SIZE_INDEX_TABLE;

	//insert the snode int the single linked list at index_table[index]
...
Artist *find_artist(char *name)
{
	...
	/* name[0]: 00000000 ~ 11111111 , if first bit is 1, its - */
	while (p != NULL && strcmp(p->name,name) < 0)
		p = p->next;
